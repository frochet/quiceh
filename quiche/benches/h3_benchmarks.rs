...
fn bench_h3_quicv3(
    ...
    loop {
        match s.client.poll_v3(&mut s.pipe.client, &mut s.pipe.client_app_buffers) {
            Ok((stream_id, quiche::h3::Event::Data)) => {
        ...
        |b, sendbuf| {
            b.iter_batched_ref(
                || {
                    ...
                    let mut s = Session::with_configs(&mut config_v3, &h3_config).unwrap();
                    s.handshake().unwrap();
                    ...
                    for stream_id in response_map.keys() {
                        s.server.send_response(&mut s.pipe.server, *stream_id, &resp, false).unwrap();
                        s.server.send_body(&mut s.pipe.server, *stream_id, &sendbuf, true).unwrap();
                    }
                    // let's catch flying packets
                    let flight =
                        quiche::testing::emit_flight(&mut s.pipe.server)
                            .unwrap();
                    (s, flight)
                },
    ...
    group.bench_with_input(
        ...
        |b, sendbuf| {
            b.iter_batched_ref(
                || {
                    // init called for each run
                    let mut s = Session::with_configs(&mut config_v1, &h3_config).unwrap();
                    s.handshake().unwrap();
                    let mut response_map = StreamIdHashMap::default();
                    let resp = vec![
                        Header::new(b":status", b"200"),
                ...
                |(ref mut s, flight, response_map)| bench_h3(s, flight, response_map),
                BatchSize::SmallInput,
            )
        },
    );
