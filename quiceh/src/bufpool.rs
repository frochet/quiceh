//! This module provides a global, lazily-initialized buffer pool for
//! `StreamChunk` objects.
//!
//! The buffer pool is designed to optimize memory allocation and reuse for
//! `StreamChunk`s, which are used for receiving stream data. It can be
//! initialized with a specific maximum memory size at application startup, or
//! it will lazily initialize with a default size upon first access.
//!
//! # Usage
//!
//! To explicitly initialize the buffer pool with a custom maximum memory size
//! (in bytes), call the `init` function once at the beginning of your
//! application:
//!
//! ```
//! use quiceh::bufpool;
...
//! ```
//!
//! If `init` is not called, the buffer pool will be automatically initialized
//! with a default size (1 GiB) when `pool_or_default()` is first invoked.
//!
...
pub fn pool_or_default() -> &'static BufPool {
    POOL.get_or_init(|| {
        BufPool::new(MAX_MEMORY_POOL / DEFAULT_CHUNK_LEN, DEFAULT_CHUNK_LEN)
    })
}
