...
fn bench_stream_send(
    ...
    // each sample bench outside of the timed function, or the flamegraph
    // would be unreadable.
    if *stream_id > 1_000 {
    ...
    <F as BufFactory>::Buf: BufSplit,
{
    ...
    *stream_id += 4;
    pipe.client
        .stream_send_zc(*stream_id, benchbuf.clone(), Some(10000), true)
        .unwrap();
    loop {
        let (write, send_info) =
            match pipe.client.send_on_path(outbuf, None, None) {
...
fn bench_sender(
    ...
    let mut pipe = Pipe::<BenchBufFactory>::with_config(config).unwrap();
    pipe.handshake().unwrap();
    stream_id = 0;
