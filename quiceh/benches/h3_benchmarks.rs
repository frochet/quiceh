...
use quiceh::h3::testing::Session;
use quiceh::h3::Header;
...
type StreamIdHashMap<V> = std::collections::HashMap<u64, V, BuildStreamIdHasher>;

// Process 157 packets at a time to roughly match the MAX RCV_BUF
pub const BATCH_PACKETS_SIZE: usize = 157;

fn bench_h3(
    s: &mut Session, flight: &mut Vec<(Vec<u8>, quiceh::SendInfo)>,
        ...
        |b, sendbuf| {
            b.iter_batched_ref(
                || {
                    ...
                    let mut s =
                        Session::with_configs(&config_v3, &h3_config).unwrap();
                    s.handshake().unwrap();
                    ...
                    let flight =
                        quiceh::testing::emit_flight(&mut s.pipe.server).unwrap();
                    (s, flight)
                },
        ...
        |b, sendbuf| {
            b.iter_batched_ref(
                || {
                    // init called for each run
                    let mut s =
                        Session::with_configs(&config_v1, &h3_config).unwrap();
                    s.handshake().unwrap();
                    let mut response_map = StreamIdHashMap::default();
                    let resp = vec![
                        Header::new(b":status", b"200"),
